...
const ChatProfile = ({ chat }) => {
  ...
  return (
    <li
      className={`${chat?.unread?.msg_count > 0 && "bg-unread-message"} listing-box message-user-list position-relative border-top-0 px-12 d-flex align-items-center justify-content-between ${currentChannel?.id === chat.id && "active-chat"
        }`}
      key={chat.id}
      onClick={() => { currentChannelHandler(chat), openUserChat()}}
    >
      <div className="d-flex align-items-center w-75">
        {chat?.type !== "P" ? (
          <ProfileImage
            key={fullName}
            name={fullName}
            src={user?.profilePicURL || user?.companyDetail?.logo || user?.instituteDetails?.logo}
            size={24}
          />
        ) : (
        ...
        <div
          className={`position-relative ${chat.type === "P" ? "pl-2" : ""
