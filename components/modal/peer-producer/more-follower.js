...
import { getFollowerListing } from "store/actions/learningInstitute";
...
export const MoreFollower = () => {
  ...
  const { morefollower } = useSelector(({ ui }) => ui.modals, shallowEqual);
  const list = useSelector(
    (state) => state?.learningInstitute?.followerListPage
  );
  /*******************
  @purpose : Follower Listing 
  @Author : INIC
  ******************/
  useEffect(() => {
    if (instituteId && morefollower) {
      const data = {
        page,
        pagesize,
        searchText,
      ...
      dispatch(getFollowerListing(data, true));
    }
  }, [instituteId, morefollower, page, searchText]);
