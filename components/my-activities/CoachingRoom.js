    ...
    <Card.Body className="m-0 p-0 mb-4">
      ...
      <Row className="  px-2 px-sm-3 py-3  row-col-10 three-grid-spacing ">
        ...
        myLearningCoaching_roomList?.rows.length > 0 ? (
          myLearningCoaching_roomList?.rows.map((virtualEventData, index) => {
            return (
              <Fragment key={index}>
                <Col
                  lg={4}
                  sm={6}
                  key={virtualEventData?.id}
                  ...
                  <Card className="secondary-card abstract-card-v2 h-100">
                    <Card.Body className="d-flex flex-column h-100">
                      <Link
                        route={
                          "/my-learning/room/" +
                          virtualEventData?.virtualEventDetails[0]?.id
                        }
                      >
                        <a>
                          <div className="position-relative pointer">
                            <picture onContextMenu={(e) => e.preventDefault()}>
                              <source
                                srcSet={
                                  virtualEventData?.virtualEventDetails[0]
                                    ?.imageURL
                                }
                                type="image/png"
                              />
                            ...
                            <span
                              ...
                              onClick={() =>
                                showModalWithValue("mylearning", {
                                  sourceId:
                                    virtualEventData?.virtualEventDetails &&
                                    virtualEventData?.virtualEventDetails[0]
                                      ?.id,
                                  sourceType: "coaching-room",
                                })
                              }
                            >
                              Leave Rating
      ...
      {total?.totalCoaching > pagesize?.pagesizeCoaching && (
        <div className="text-right d-flex border-top border-geyser people-tab-view-all-button mb-4">
          <small
            ...
            onClick={() => {
              setPagesize({
                ...pagesize,
                pagesizeCoaching: pagesize?.pagesizeCoaching + 3,
              });
            }}
          >
