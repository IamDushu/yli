...
const ConnectionTableRow = ({ row, getFilteredList }) => {
  ...
  const { users, id, goals, connectionUserId, mutualCount } = row;
  ...
  const getPopover = () => {
    return (
      ...
      >
        <div onMouseLeave={handleClosePopover}>
          <UserCard
            name={userName}
...
const GrowthConnectionsTable = ({ growthConnections, getFilteredList }) => {
  const [lang] = useTranslation("language");
