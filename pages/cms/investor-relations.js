...
import { getInvesterRelationInfo } from "store/actions";
...
const CookiesPolicy = () => {
  ...
  useEffect(() => {
    ...
    dispatch(getInvesterRelationInfo(currentLanguage));
  }, []);

  const { investorRelationsInfo } = useSelector(
    ({ investorRelations }) => investorRelations
  );

  function createMarkup() {
    if (investorRelationsInfo.content) {
      return {
        __html: investorRelationsInfo.content,
      };
    }
  }
