		...
		<div ref={modalRef} style={modalStyles}>
			<div className="add-edit-event-wrapper">
				<div className="add-edit-event-header">
					<CloseIcon onClick={handleCancel} className="mr-2" />
				</div>
				<Formik
					enableReinitialize
					initialValues={{
							...
							return (
								<div>
									<Form className="add-edit-event-form">
										<Grid className="pl-16 pr-16" container>
											<Grid xs={12}>
												<Field name="title">
													{({ field }) => {
														...
														return (
															<React.Fragment>
																<TextField size="small" label="Outlined" variant="outlined" label="Enter Title" onChange={e => setFieldValue("title", e.target.value)} value={field.value} />
										...
										<Grid className="mb-2 pl-16 pr-16" container>
											<Grid xs={12}>
															...
															<React.Fragment>
																<div className="d-flex justify-content-between align-items-center">
																	...
																	<LocalizationProvider dateAdapter={AdapterDayjs}>
																		<DateTimePicker
																			label="Start"
																			value={dayjs(field.value[0].utc().format())}
																			onChange={(newValue) => {
																				setFieldValue([moment(newValue.format()), field.value[1]])
																			}}
																		/>
																	...
																	<LocalizationProvider dateAdapter={AdapterDayjs}>
																		<DateTimePicker
																			label="End"
																			value={dayjs(field.value[1].utc().format())}
																			onChange={(newValue) => {
																				setFieldValue([field.value[0], moment(newValue.format())])
																			}}
																		/>
																	</LocalizationProvider>
																	{/* <RangePicker
												...
												<Field name="eventUsers">
													{({ field }) => {
														return (
															<React.Fragment>
																<div className="d-flex justify-content-between align-items-center">
																	<SvgIcon className="mr-1" fontSize="small">
																		{selectParticipants()}
																	</SvgIcon>
										...
										<Grid className="pl-16 pr-16 pb-12" container>
											<Grid xs={12}>
												<Field name="note">
													{({ field }) => {
														const { setFieldValue } = useFormikContext();
														return (
															<React.Fragment>
																<TextField
																	...
																	onChange={e => setFieldValue("note", e.target.value)} value={field.value}
																/>
																<div className="event-text-limit"><span>{field.value.length} / <b>100</b></span></div>
															</React.Fragment>
														)
													...
													return (
														<React.Fragment>
															<Select
																defaultValue={field.value}
																value={field.value}
																onChange={(e) => setFieldValue('reminder', e.target.value)}
																size="small"
															>
																{
										...
										<div className="add-edit-event-submit">
											<Button variant="contained" size="medium" htmlType="submit">Save</Button>
										</div>
									</Form>
								</div>
							);
